...
form.addEventListener('keyup', function(){
    ...
    displayItems = products.filter(function(product){
        return product.title.toLowerCase().includes(inputValue)
    })
    showProducts(displayItems)
...
function showProducts(list){
    const displayItems = list.map(function(item){
        return `<div class="product" id= "${item.title}">
    ...
    productsList.innerHTML = displayItems;
}


function displayMenuItems (){
    const companies = products.reduce(function(menu, item){
        if(!menu.includes(item.company)) {
            menu.push(item.company);
        }
        return menu;
    },["all"]);

    const categoryLinks = companies.map(function(category) {
        return `<li><button type = "button" class="link" data-id = ${category}>${category}</button></li>`
    }).join("");
    buttonContainer.innerHTML = categoryLinks;
    const links = document.querySelectorAll(".link");

    links.forEach(function(link){
        link.addEventListener("click", function(e){
            const company = e.currentTarget.dataset.id;
            const productCategory = products.filter(function(menuItem){
